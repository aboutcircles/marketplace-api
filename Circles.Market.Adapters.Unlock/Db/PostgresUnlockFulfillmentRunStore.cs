...
public interface IUnlockFulfillmentRunStore : IFulfillmentRunStore;

public sealed class PostgresUnlockFulfillmentRunStore : IUnlockFulfillmentRunStore
{
    private readonly string _connString;

    public PostgresUnlockFulfillmentRunStore(string connString)
    {
        ...
        CancellationToken ct)
    {
        ...
        int staleMinutes = 10;
        var staleEnv = Environment.GetEnvironmentVariable("UNLOCK_FULFILLMENT_STALE_MINUTES");
        if (!string.IsNullOrWhiteSpace(staleEnv) && int.TryParse(staleEnv, out var parsed) && parsed > 0)
            staleMinutes = parsed;

        bool allowStartedTakeover = false;
        var allowEnv = Environment.GetEnvironmentVariable("UNLOCK_FULFILLMENT_ALLOW_STARTED_TAKEOVER");
        if (!string.IsNullOrWhiteSpace(allowEnv) && bool.TryParse(allowEnv, out var parsedAllow))
...
INSERT INTO unlock_fulfillment_runs(chain_id, seller_address, payment_reference, order_id, status, updated_at)
...
ON CONFLICT (chain_id, seller_address, payment_reference) DO NOTHING;";
        ...
        await using (var ins = conn.CreateCommand())
        {
            ...
            if (rows == 1) return (true, "started");
        }

        const string takeoverSql = @"
UPDATE unlock_fulfillment_runs
SET status='started',
    updated_at=now(),
    completed_at=NULL,
    last_error=NULL,
  ...
  AND (
      ...
      OR (@allowStartedTakeover AND status='started' AND updated_at < now() - (@staleMinutes || ' minutes')::interval)
        ...
        await using (var take = conn.CreateCommand())
        {
            ...
            if (updated == 1) return (true, "started");
        }
