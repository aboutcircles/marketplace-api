...
On startup the order and payment stores ensure their ASCII identifier columns (payment references, tx hashes, order ids, gateway/payer/seller/buyer addresses, SKUs, status strings) use `COLLATE "C"` — raw byte ordering that is independent of the OS locale library. This immunizes the unique/lookup indexes on those columns against **glibc collation drift**: when the C library's text-ordering rules change (e.g. a base-image upgrade) on a database whose default collation is locale-based (`en_US.utf8`) and whose `datcollversion` tracking is off, a btree built under the old ordering can navigate to the wrong leaf — so a uniqueness check misses an existing key and duplicate rows slip in under a still-"valid" unique index. That exact failure stranded fully-paid orders in production.
*   **Idempotent + self-discovering**: only text columns not already `"C"` are converted, so it applies once and then no-ops on every subsequent start; text columns added to those tables later are converted automatically.
*   **Best-effort, non-fatal**: the conversion takes a brief `ACCESS EXCLUSIVE` lock to rebuild dependent indexes (bounded by `lock_timeout = 3s`). If it can't apply on a given start it logs a warning, increments `marketplace_schema_collation_migration_failures_total{store}`, and retries next start — it never blocks boot. The core schema (table/column/index creation) remains fatal-on-failure as before. **Alert on that counter**: a nonzero value means identifier columns are still on a locale collation, and in particular a duplicate-key failure during the index rebuild indicates live duplicate rows under a unique index (drift has already struck) — investigate for duplicates, do not ignore.
